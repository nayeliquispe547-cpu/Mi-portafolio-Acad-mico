...
async function crearEntregaDesdeArchivo(file) {
  try {
    // Usamos FormData para enviar el archivo directamente a Catbox.moe
    const formData = new FormData();
    formData.append("reqtype", "fileupload");
    formData.append("fileToUp", file);

    // Hacemos la petición fetch a la API pública y libre de Catbox
    ...
    // Catbox responde directamente con el enlace web público del archivo subido
    ...
    return {
      ...
      dataUrl: urlPublica.trim(),
      blobUrl: urlPublica.trim(),
      urlPublica: urlPublica.trim()
    };
  } catch (error) {
