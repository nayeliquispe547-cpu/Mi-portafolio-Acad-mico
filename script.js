...
function getSupabaseClient() {
  return null; // Desactivado para evitar errores de red externos
}
...
function guardarDatos(datos) {
  try {
    ...
    alert("El almacenamiento local se llenó. Intenta subir archivos más ligeros o comprimidos.");
  }
}
...
function comprimirImagen(file, maxWidth = 1000, quality = 0.65) {
...
async function crearEntregaDesdeArchivo(file) {
  try {
    // Si es imagen, la comprimimos primero para optimizar espacio local
    let resultadoUrl = null;
    if (file.type && file.type.startsWith("image/")) {
      resultadoUrl = await comprimirImagen(file);
    }
    
    // Si no se pudo comprimir o no es imagen, se lee como DataURL nativo
    if (!resultadoUrl) {
    ...
    console.error("Error al procesar el archivo localmente:", error);
